  ...
  return (
    <div
      ...
      onClick={() => {
        ...
        if (ProjectData[props.num].iphone.length !== 0)
          deviceDispatch({ type: "CHANGE_IPHONE" });
        else if (ProjectData[props.num].tablet.length !== 0)
          deviceDispatch({ type: "CHANGE_TABLET" });
        else if (ProjectData[props.num].computer.length !== 0)
          deviceDispatch({ type: "CHANGE_COMPUTER" });
      }}
    />
  );
};
